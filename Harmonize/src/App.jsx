...
function App() {
  ...
  const getAlbumCover = async (platform, sourceId) => {
    ...
    try {
      if (platform === 'youtube') {
        ...
        const res = await fetch(`/api/spotify/track/${sourceId}`);
  ...
  const fetchCurrentPlaying = async (id) => {
    try {
      const res = await fetch(`/api/rooms/${id}/current-playing`);
      const data = await res.json();
      if (res.ok) {
  ...
  const updateCurrentPlaying = async (index) => {
    if (!roomId) return;
    try {
      await fetch(`/api/rooms/${roomId}/current-playing`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (
      ...
      if (roomId && currentPlaying >= 0) {
        try {
          ...
          await fetch(`/api/rooms/${roomId}/queue/${currentPlaying}/most-recent-change`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleConfirmLeave = async () => {
    ...
    if (isAdmin) {
      ...
      try {
        await fetch(`/api/rooms/${roomId}/remove-user`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
